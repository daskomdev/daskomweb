  ...
  <div class="main_container bg-green-900 w-full h-full overflow-hidden">
    ...
    <div class="absolute w-120 z-20 h-48full bottom-0 right-0 animation-enable"
      ...
      <div class="w-full h-full animation-enable overflow-y-auto"
        ...
        <div v-if="RankingPriviledge.includes(currentUser.role_id) || RankingPriviledgePriviledge == 'all'">
          <div class="w-full p-4 h-24 flex select-none cursor-pointer hover:text-white animation-enable"
          ...
          <!-- All Laporan List -->
            <div>
                <transition-group name="history-list" tag="div">
                    <div v-for="history in allHistory.filter( function(item){return (item.modul_id===choosenModulID);} )" v-bind:key="history.id" 
...
export default {
  ...
  methods: {
    ...
    travel: function($whereTo){
      ...
      setTimeout(
        function() {
          globe.$inertia.replace('/'+ $whereTo +'?comingFrom=allLaporan&position='+globe.$refs.menu.scrollTop);
        }, 501); 
    },
